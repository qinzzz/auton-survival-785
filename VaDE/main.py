...
if __name__ == '__main__':
    ...
    train_loader, val_loader = get_support(args.datadir,args.batch_size)
    ...
    vade.pre_train(train_loader,pre_epoch=50)
    ...
    for epoch in epoch_bar:
        ...
        writer.add_scalar('loss',L/len(train_loader),epoch)
        # writer.add_scalar('acc',cluster_acc(pre,tru)[0]*100,epoch)
        writer.add_scalar('lr',lr_s.get_lr()[0],epoch)
        print("predicted:", pred)
        epoch_bar.write('Loss={:.4f},LR={:.4f}'.format(L/len(train_loader), lr_s.get_lr()[0]))
